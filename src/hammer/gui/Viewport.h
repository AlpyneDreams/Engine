...
{
    struct Viewport : public GUI::Window
    {
        Viewport() : GUI::Window(ICON_MC_IMAGE_SIZE_SELECT_ACTUAL, "Viewport", 512, 512, true, ImGuiWindowFlags_MenuBar) {}
        ...
        auto GetTexture(DrawMode mode)
        {
            switch (mode) {
                default: case DrawMode::Shaded:
                    return editor::Editor.rt_SceneView->GetTexture();
                case DrawMode::Depth:
                    return editor::Editor.rt_SceneView->GetDepthTexture();
            }
        }
    ...
    // UI //
        ...
        bool CheckResize()
        {
            ...
            if (size.x != width || size.y != height)
            {
                ...
                editor::Editor.ResizeViewport(width, height);
